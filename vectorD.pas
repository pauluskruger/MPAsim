...
procedure CVwriteM(N,L : integer;A : cvec);

//Real Matrix
Procedure RMsetsize(N,L : integer;var M : tmat);
procedure RMwrite(N,L : integer;A : tmat);
procedure RMwriteE(N,L : integer;A : tmat);

//Complex Matrix
Procedure CMsetsize(N,L : integer;var M : cmat);
Procedure CMclearsize(N : integer;var M : cmat);
...
function CMelim2(N : integer;M : cmat;a,b,c : integer) : integer;
...
begin
 for y:=0 to L-1 do begin
  for x:=0 to N-1 do write(A[x,y]:5:2);
  writeln;
  end;
end;
...
begin
 for y:=0 to L-1 do begin
  for x:=0 to N-1 do writeEng(A[x,y]);
  writeln;
  end;
end;
...
begin  
 for y:=0 to L-1 do begin
   for x:=0 to N-1 do write(A[x*L+y,1]:7:3,'+',A[x*L+y,2]:7:3,'i');
   writeln;end;
 end;
...
//cmwrite(N,N,M);
for x:=a to b do begin
 if (M[x,x,1]=0) and (M[x,x,2]=0) then begin //do rows swap
    y:=x;
    while (y<=c) and (M[x,y,1]=0) and (M[x,y,2]=0) do inc(y);
    if y>c then begin 
//      writeln('x=',x,' c=',c);
      CMelim2:=x;exit;
      end;
    for i:=0 to b do begin
      z:=M[i,x];M[i,x]:=M[i,y];M[i,y]:=z;
      end;
    end;
 z2:=-1/M[x,x];
  for y:=x+1 to c do if ((M[x,y,1]<>0) or (M[x,y,2]<>0)) then begin
   z:=z2*M[x,y];
//   write('Z=');cwrite(z);writeln;
//   for i:=0 to N-1 do cadd(M[i,y],z*M[i,x]);
   for i:=0 to a-1 do if ((M[i,x,1]<>0) or (M[i,x,2]<>0)) then  cadd(M[i,y],z*M[i,x]);
...
for x:=b downto a+1 do begin
 if (M[x,x,1]=0) and (M[x,x,2]=0) then begin;CMelim2:=x;exit;end;
 z2:=-1/M[x,x]; 
  for y:=x-1 downto a do if ((M[x,y,1]<>0) or (M[x,y,2]<>0)) then begin
   z:=z2*M[x,y];
   for i:=0 to a-1 do cadd(M[i,y],z*M[i,x]);
...
x : integer;
begin
for x:=0 to N-1 do setlength(M[x],0);
setlength(M,0);
end;
